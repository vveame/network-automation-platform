...
# Optional SNMPv3 startup for FRR containers.
# SNMP must never prevent the router from starting.
...
echo "[SNMP] Starting snmpd with SNMPv3 authPriv on UDP/${SNMP_AGENT_PORT}."
...
if snmpd -Lo -c "$SNMP_CONFIG_FILE" "udp:${SNMP_AGENT_PORT}"; then
  ...
  echo "[SNMP][WARN] snmpd failed to start. Continuing router startup."
fi
