...
resource "aws_key_pair" "admin" {
  ...
  tags       = merge(var.common_tags, { Name = "${var.name_prefix}-admin-key", Role = "ec2-admin-access" })
}
...
resource "aws_instance" "tunnel_gateway" {
  ...
  tags = merge(var.common_tags, { Name = "${var.name_prefix}-tunnel-gateway", Role = "hybrid-ec2-tunnel-gateway", Tier = "public" })
}
...
resource "aws_instance" "monitoring" {
  ...
  key_name                    = local.ssh_key_name
  user_data                   = templatefile("${path.module}/user-data/monitoring.sh.tftpl", { name_prefix = var.name_prefix })
  iam_instance_profile        = try(aws_iam_instance_profile.ssm[0].name, null)
  tags                        = merge(var.common_tags, { Name = "${var.name_prefix}-monitoring", Role = "cloud-monitoring", Tier = "monitoring-ai" })
}
...
resource "aws_instance" "ai" {
  ...
  key_name                    = local.ssh_key_name
  iam_instance_profile        = try(aws_iam_instance_profile.ssm[0].name, null)
  tags                        = merge(var.common_tags, { Name = "${var.name_prefix}-ai", Role = "cloud-ai-analysis", Tier = "monitoring-ai" })
}
